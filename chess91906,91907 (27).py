...
class piece_rules:
    ...
    def __init__(self,frame,piece_move_info,button_to_move):
        ...
        print(f"turn global value = {self.turn}")
...
class turn_class:
    ...
    def white_turn(self):
        ...
        for x in black_pieces_dict.keys():
            print(f"x = {x}")
            piece_to_change = self.frame.nametowidget(name=x)
            
            turn == 1
            for x in button_dict.keys():
                button_to_change = self.frame.nametowidget(name=x)
                pywinstyles.set_opacity(button_to_change,value=0.05)
                button_to_change.lift()
            return
                
    def black_turn(self):  
        global turn 
        for x in white_pieces_dict:
            piece_to_change = self.frame.nametowidget(name=x)
            #pywinstyles.set_opacity(piece_to_change,value=0.5)
