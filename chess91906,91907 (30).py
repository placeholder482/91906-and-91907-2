...
class chess_board_create_class:
    ...
    def main_board_create_function(self):
        ...
        global frame_main_board_function
        grid_x_value = 0
        grid_y_value = 0
...
class piece_rules:
    ...
    def pawn_move_check_function(self,piece_move_loc,move_to):
        ...
        #             if move_valid == 1:
        #                   piece_moving.grid_configure(row=taking_pawn_white_right[1],column=taking_pawn_white_right[0])
        ...
        #             self.move_validity_check(move_start=location,move_end=move_to,diagonal=1,piecename=piece_move_loc[2])
        #             if move_valid == 1:
        #                                       piece_moving.grid_configure(row=taking_pawn_white_left[1],column=taking_pawn_white_left[0])
        ...
        #             self.move_validity_check(move_start=location,move_end=move_to,diagonal=1,piecename=piece_move_loc[2])
        #             if move_valid == 1:
        #                                       piece_moving.grid_configure(row=taking_pawn_black_right[1],column=taking_pawn_black_right[0])
        ...
        #             self.move_validity_check(move_start=location,move_end=move_to,diagonal=1,piecename=piece_move_loc[2])
        #             if move_valid == 1:
        #                                       piece_moving.grid_configure(row=taking_pawn_black_left[1],column=taking_pawn_black_left[0])
        ...
        if piece_move_loc[1] == "white":
                    ...
                    if (piece_current_row - move_to[1] == -1):
                        ...
                        print("please3")
                        if move_valid == 1:
                            print('fhdjfijdasofjoisafj')
                            if [move_to[1] -1,move_to[0]] in white_pieces_dict.values():
                                        move_valid = 0
                                        print("fdfasdfs")
                            else:
                                    if [move_to[1] -1,move_to[0]] in black_pieces_location_grid:
                                            print('as')
                                    else:
                                         print([move_to[1] +1,move_to[0]])
                                         print(black_pieces_location_grid)
                                         piece_moving.grid_configure(row=piece_current_row +1)
                                         if ([move_to[1],move_to[0]] in black_pieces_location_grid) == True:
                                                                piece_moving.grid_configure(row=piece_current_row)
                                         print("asfdsfasdf2")
                        else:
                             print("a")
                        #self.__init__()
            ...
            if piece_current_row == 7:
                ...
                if (piece_current_row - move_to[1] == 2):
                    ...
                    if move_valid == 1:
                        print("please2")
                        if [move_to[1] -2,move_to[0]] in white_pieces_location_grid:
                             move_valid = 0
                        if [move_to[1],move_to[0]] not in white_pieces_location_grid:
                            piece_moving.grid_configure(row=piece_current_row -2)
                    else:
                                         print("a")
            ...
            if (piece_current_row - move_to[1])== 1:
                ...
                if move_valid == 1:
                    print("adfs")
                    print([move_to[1] -1,move_to[0]] in black_pieces_dict.values())
                    if ([move_to[1] -1,move_to[0]] in black_pieces_dict.values()) == True:
                                    move_valid = 0
                    else:
                        print("please")
                        piece_moving.grid_configure(row=piece_current_row -1)
                        if ([move_to[1],move_to[0]] in white_pieces_location_grid) == True:
                            piece_moving.grid_configure(row=piece_current_row)
                        print([move_to[1],move_to[0]] in white_pieces_location_grid)
                else:
                     print("a")
                #self.__init__()
            elif move_valid == 0:
    ...
    def move_validity_check(self,move_start,move_end,diagonal,piecename):
            ...
            start_row = move_start[1]
            start_column = move_start[0]
            start_coord = [start_column,start_row]
            for x in white_pieces_dict.keys():
                  print("frame")
                  print(frame_main_board_function)
                  piece_white = frame_main_board_function.nametowidget(x)
                  piece_white_grid = piece_white.grid_info()
                  white_row = piece_white_grid["row"]
                  white_column = piece_white_grid["column"]
                  append_list = [white_column,white_row] 
                  piece_locations.append(append_list)
            for x in black_pieces_dict.keys():
                              print("frame")
                              print(frame_main_board_function)
                              pieces_black = frame_main_board_function.nametowidget(x)
                              pieces_black_grid = pieces_black.grid_info()
                              black_row = pieces_black_grid["row"]
                              black_column = pieces_black_grid["column"]
                              append_list = [black_column,black_row] 
                              piece_locations.append(append_list)
            ...
            #sprint(white_pieces_dict)
            # for x in white_pieces_dict.keys():
            #             #print(x)
            #             piece_moving = self.frame.nametowidget(name=x)
            #             if piece_moving == piecename:
            #                  print("f")
            #                  print(piecename)
            #             else:
            #                 transfer_grid_loc = piece_moving.grid_info()
            ...
            #                 #print(f"piece location = {piece_locations}")
            #                 #print(f"x = {x}")
            if move_start_true not in piece_locations:
                pass
            else: 
                 piece_locations.remove(move_start_true)
            row_diffrence = move_end[1] - start_coord[1]
            column_diffrence = move_end[0] - start_coord[0]
            ...
            if diagonal == 0:
                ...
                if row_change == True:
                    minus = False
                    # if ([start_coord[0],start_coord[1]]) == ([move_end[1],move_end[0]]) == True:
                    #                      print("this one")
                    #                      move_valid = 1
                    #                      return True
                    loop_ran = False
                    ...
                    print(f"check = {(start_row,start_column)}{(move_end[1],move_end[0])}")
                    print(step_of_column,step_of_rows)
                    a = [current_row,current_column]
                    print("avdfas")
                    print(([start_coord[1],start_coord[0]]) == ([move_end[1],move_end[0]]))
                    #print((current_row,current_column) != (move_end[1],move_end[0]))
                    if minus == False:
                            while (([start_coord[1],start_coord[0]]) == ([move_end[1],move_end[0]])) == False:
                                  location_check = [start_coord[1],start_coord[0]]
                                  #print("check running")
                                  if location_check in piece_locations:
                                        print(f"error point = {location_check}")
                                        move_valid = 0
                                        return False
                                  start_coord[1] += 1
                            move_valid = 1
                            ...
                            #                                print("a")
                            #                                print(start_coord)
                            #                                if [start_coord[1],start_coord[0]] in piece_locations:
                            #                                      print("a")
                            #                                      move_valid = 1
                            # move_valid = 1
                            # return True
                            # while ([start_coord[1],start_coord[0]]) != ([move_end[1],move_end[0]]):
                            #      print("check")
                            #      loop_ran = True
                            #      #print(F"piece locations = {piece_locations}")
                                 
                            #      list_check = [start_coord[0],start_coord[1]]
                            #      print("list_check")
                            #      print(list_check)
                    ...
                    if minus == True:
                                ...
                                print(([start_coord[1],start_coord[0]]) == ([move_end[1],move_end[0]]))
                                while (([start_coord[1],start_coord[0]]) == ([move_end[1],move_end[0]])) == False:
                                                           location_check2 = [start_coord[1],start_coord[0]]
                                                           print("check running  minus")
                                                           print(f"location check = {location_check2}")
                                                           if location_check2 in piece_locations:
                                                                 print(f"error point = {location_check2}")
                                                                 move_valid = 0
                                                                 return False
                                                           start_coord[1] -= 1
                                move_valid = 1
                            ...
                            while loop_amount < 10:
                                ...
                                if check_list == move_end:
                                    ...
                                    while loop_anoumt_move_through < 10:
                                        move_through_check = [move_start[1]-loop_anoumt_move_through,move_start[0]-loop_anoumt_move_through]
...
class file_saving:
    ...
    def game_save(self):
        ...
        for x in self.frame.winfo_children():
             ...
             value_to_add_to_json_text = str(button_name.cget("text"))
             ...
             for b in black_pieces_dict.keys():
                if button_name == b:
                    team = "black"
             value_to_add_to_json_text = (f"{value_to_add_to_json_text}")
             ...
             with open("gamedata.txt","a") as f:
                f.write(button_name_x)
                f.write("split")
                f.write("[")
                
                f.write("'")
                f.write(value_to_add_to_json_text)
                f.write("'")
    ...
    def game_load(self):
        ...
        with open("gamedata.txt","r") as r:
            ...
            #         #print(game_data_load_dictionary)
                    dictionary_list = game_data_load.rsplit("split")
                    widgetname =  dictionary_list[0]
                    widgetdata = dictionary_list[1]
               #game_data_load_dictionary[game_data_load.rsplit(":")] = [game_data_load.split(":")]
                    print(widgetname)
                    game_data_load_dictionary[widgetname] = widgetdata
                    print(game_data_load_dictionary)
                    for x in game_data_load_dictionary.keys():
                          print(x)
                          widgetname_data = frame_main_board_function.nametowidget(x)
                          data_transfer = game_data_load_dictionary[x]
                          #print(data_transfer[2])
                    
                          widgetdata = widgetdata
                          print(f"widget = {widgetdata}")
                          a = ast.literal_eval(widgetdata)
                          a = list(a)
                          list_data = widgetdata
                          print(list_data[4])
                          print(a[1])
                          widgetname_data.config(text=a[0])
                          print(a[1][0])
                          widgetname_data.grid_configure(row=a[1][0],column=a[1][2])

                    
                          #print(ast.literal_eval(data_transfer[2]))
               #print(split_data)
               #game_data_load = dict(game_data_load)
